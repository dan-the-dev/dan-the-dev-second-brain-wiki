---
title: Abitudini attive
updated: 2026-08-30
tags: [habits, wiki]
---

# Abitudini attive

> **Ridefinizione 16/8/2026:** set di abitudini completamente rinnovato insieme ai goal — vedi [[../raw/2026-H2-redefinition-2026-08-16|ridefinizione]]. Le abitudini precedenti (W24-W32) sono archiviate: la loro storia resta consultabile in `raw/habits-log/`, ma da qui in poi si traccia il set sotto, con streak azzerati.
>
> **Check-in W33 (10-16/8):** primo check-in dopo la ridefinizione — coincide con il giorno stesso della ridefinizione (16/8) e con un gap totale di journal dal 7 al 16/8. Nessuna abitudine verificabile, streak confermati a 0. Dettaglio in [[../raw/habits-log/2026-W33-habits|raw/habits-log/2026-W33-habits]].
>
> **Gap W34 (17-23/8):** nessun check-in creato quella settimana. **Check-in W35 (24-30/8):** un solo dump journal nel periodo (25/8, checkpoint uscita Muffin). Prima abitudine verificata dalla ridefinizione: "Checklist chiusura Muffin (handover)" mantenuta → streak 1. Tutte le altre restano non tracciabili questa settimana. Dettaglio in [[../raw/habits-log/2026-W35-habits|raw/habits-log/2026-W35-habits]].
>
> **Fonti per i check-in:** il journal `career-coach` copre principalmente l'area professionale. Per le aree Salute, Casa, Studio, Finanziario e Side project, in assenza di segnali nel journal, lo stato viene aggiornato sulla base di quanto Dan comunica esplicitamente in conversazione (come il giro del 23/8) — non è un problema da risolvere con una fonte scritta aggiuntiva, è il flusso previsto.
>
> **Schema settimanale — slot mattutino (mar-ven):** lunedì in ufficio, martedì-venerdì da casa. 1 ora fissa al giorno (idealmente 8:00-9:30, orario che può spostarsi a dopo lavoro) suddivisa in 4 slot: 2 pratica coding agent open source su Mister OS (Goal 1.1 + 11), 1 studio teoria AI/LLM (Goal 8), 1 produzione contenuti LAP il giovedì o venerdì (Goal 4). Dettaglio in [[../raw/2026-H2-redefinition-2026-08-16#Schema settimanale — slot mattutino (mar-ven)|ridefinizione]].

## 🧠 Professionale

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Slot mattutino: pratica coding agent open source su Mister OS | Goal 1.1 + 11 | 2x settimana (mar-ven) | 🆕 Nuova | 0 |
| Slot mattutino: produzione contenuti LAP (newsletter/podcast/blog a rotazione) | Goal 4 | 1x settimana (gio o ven) | 🆕 Nuova | 0 |
| Review costi/token Claude Code | Goal 1.2 | Mensile | 🆕 Nuova | 0 |
| Step avanti su self-hosting (Coolify → LLM Wiki → N8N → Postiz → Cal.com/Passbolt) | Goal 1.3 | Settimanale | 🆕 Nuova | 0 |
| Documentare 1 pratica XP+AI nel journal | Goal 2 | Settimanale | ⚠️ Riparte da 0 (era a 0 streak da 4 settimane, W29-W32, nel set precedente) | 0 |
| 1 ora studio harness/loop o nuovo tool | Goal 2 | Ogni 2 settimane | 🆕 Nuova | 0 |
| Applicare AI a un tema QA/infra/prodotto | Goal 2 | Mensile | 🆕 Nuova | 0 |
| Checklist chiusura Muffin (handover) | Goal 3 | Settimanale (fino al 14/9) | 🟢 Mantenuta (25/8) | 1 |
| Retro settimanale onboarding Levels | Goal 3 | Settimanale (dal 14/9) | ⏸️ Non ancora attiva | 0 |
| Check-in avanzamento onboarding | Goal 3 | Mensile (dal 14/9) | ⏸️ Non ancora attiva | 0 |
| Pubblicare 1 articolo blog LAP | Goal 4 | Mensile | 🆕 Nuova | 0 |
| Pubblicare 1 newsletter LAP | Goal 4 | Settimanale | 🆕 Nuova | 0 |
| Pubblicare 1 episodio podcast LAP | Goal 4 | Settimanale | 🆕 Nuova | 0 |

## ⚽ Ardor

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Check-in con le ragazze sul post partita | Goal 5 | Per partita | ❓ Non tracciabile (probabile pausa stagionale) | 0 |
| Coordinamento stories con gruppo dirigenti | Goal 5 | Weekend (in stagione) | 🆕 Nuova | 0 |
| Aggiornare la LLM Wiki dopo ogni allenamento | Goal 5.2 | Da definire (da quando iniziano gli allenamenti, settembre) | 🆕 Nuova — goal formalizzato il 23/8 | 0 |
| Aggiungere nuovo esercizio al catalogo | Goal 5.2 | Per esercizio introdotto | 🆕 Nuova — goal formalizzato il 23/8 | 0 |

## 💪 Salute

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| 12.000 passi al giorno | Goal 6 | Giornaliero | 🆕 Nuova (era 10k) | 0 |
| Allenamento 19:00-20:30 post-Juniores | Goal 6 | 3x settimana | 🆕 Nuova | 0 |
| Sessione miglioramento cucina | Goal 6 | Settimanale | 🆕 Nuova | 0 |
| Test pasto con alternative pronte low-cal | Goal 6 | Settimanale | 🆕 Nuova | 0 |
| Prenotare/fare visita apnee | Goal 7 | Azione singola (non ricorrente) | 🔴 Ancora da fare — scadenza luglio superata | 0 |

## 📚 Crescita personale / Studio

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Slot mattutino: studio teoria AI/LLM | Goal 8 | 1x settimana (mar-ven) | 🆕 Nuova | 0 |
| Nota su Recall dopo argomento/capitolo | Goal 8 | Per argomento | 🆕 Nuova | 0 |

## 🏠 Casa

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Mezza giornata domenica — pulizie grosse | Goal 9 | Settimanale | 🆕 Nuova | 0 |
| Mantenimento leggero infrasettimanale | Goal 9 | 2-3x settimana | 🆕 Nuova | 0 |
| Check ottimizzazione spesa/giri | Goal 9 | Mensile | 🆕 Nuova | 0 |
| Presenza/supporto a Giulia in agility | Goal 9 | Settimanale | 🆕 Nuova | 0 |
| Sync 15 min con Giulia (pianificazione pulizie/spesa/agility) | Goal 9 | Settimanale | 🆕 Nuova | 0 |

## 💰 Finanziario

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Check mensile scadenze con CFO personale | Goal 10 | Mensile | 🆕 Nuova | 0 |
| Saldo debito papà alla ricezione TFR | Goal 10 | Azione singola | ⏸️ In attesa TFR Muffin | 0 |
| Versamento automatico fondo emergenza | Goal 10 | Mensile (dopo saldo debiti) | ⏸️ Non ancora attiva | 0 |

## 💻 Side project

| Abitudine | Goal collegato | Frequenza | Status | Streak |
|---|---|---|---|---|
| Mister OS | Goal 11 | — | Tempo coperto dallo slot mattutino "pratica coding agent" (vedi Professionale, Goal 1.1 + 11) | 0 |
| Tool asta fantacalcio | Goal 12 | Da definire | 🆕 Nuova — goal formalizzato il 23/8, cadenza ancora da concordare con Dan (possibile condivisione slot con Goal 1.1) | 0 |
