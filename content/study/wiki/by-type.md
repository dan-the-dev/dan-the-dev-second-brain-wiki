---
title: "By Type"
type: by-type
updated: 2026-08-19
---

# By Type

Vista dell'intero archivio compilato organizzata per tipo di contenuto, con i topic a cui ciascun elemento è agganciato. Per la vista organizzata per argomento si veda [[topics/agile-software-development|Topics]]; per l'archivio grezzo importato da Recall si veda [[knowledge-archive|Archivio Recall importato]].

**Totale: 75 contenuti** (25 book, 33 article, 9 video, 3 conference, 3 podcast, 2 course).

### Book (25)

- **Accelerate** — Nicole Forsgren, Jez Humble, Gene Kim — [[content/book/accelerate|pagina]] — tech-lead, agile-software-development, devops
- **Agile Technical Practices Distilled** — Pedro M. Santos, Marco Consolaro, Alessandro Di Gioia — [[content/book/agile-technical-practices-distilled-a-learning|pagina]] — agile-software-development, technical-excellence, ddd
- **Clean Code: A Handbook of Agile Software Craftsmanship** — Robert C. Martin — [[content/book/clean-code-a-handbook-of-agile-software|pagina]] — agile-software-development, technical-excellence, clean-code
- **Domain-Driven Design: Distilled** — Vaughn Vernon — [[content/book/domain-driven-design-distilled|pagina]] — agile-software-development, ddd
- **Endymion Spring** — Matthew Skelton — [[content/book/endymion-spring|pagina]] — _non taggato_
- **Extreme Programming: Explained** — Kent Beck — [[content/book/extreme-programming-explained|pagina]] — extreme-programming
- **The Fearless Organization** — Amy C. Edmondson — [[content/book/fearless-organization-creating-psychological|pagina]] — psychological-safety, leadership
- **From Objects to Functions** — Uberto Barbini — [[content/book/from-objects-to-functions-build-your-software|pagina]] — software-development
- **Implementing Lean Software Development** — Mary Poppendieck, Tom Poppendieck — [[content/book/implementing-lean-software-development|pagina]] — tech-lead, agile-software-development, lean, product-development
- **Lean Inception** — Paulo Caroli — [[content/book/lean-inception|pagina]] — agile-software-development, lean
- **Lean Software Development: An Agile Toolkit** — Mary Poppendieck, Tom Poppendieck — [[content/book/lean-software-development-an-agile-toolkit|pagina]] — lean, agile-software-development
- **Non-Violent Communication** — Marshall B. Rosenberg — [[content/book/non-violent-communication-the-best-way-to-connect|pagina]] — leadership, psychological-safety, nonviolent-communication
- **Peopleware: Productive Projects and Teams** — Tom DeMarco, Timothy Lister — [[content/book/peopleware-productive-projects-and-teams|pagina]] — tech-lead, leadership
- **Practice Perfect** — Doug Lemov, Erica Woolway, Katie Yezzi — [[content/book/practice-perfect-42-rules-for-getting-better-at|pagina]] — personal-growth, coaching
- **Practices of an Agile Developer** — Venkat Subramaniam, Andy Hunt — [[content/book/practices-of-an-agile-developer|pagina]] — agile-software-development, technical-excellence
- **Professional PHP** — Patrick Louys — [[content/book/professional-php-building-maintainable-and-secure|pagina]] — software-development, php
- **Programming Elixir 1.6** — Dave Thomas — [[content/book/programming-elixir-1-6|pagina]] — software-development, elixir
- **Refactoring** — Martin Fowler — [[content/book/refactoring-improving-the-design-of-existing-code|pagina]] — agile-software-development, technical-excellence, refactoring
- **Slack** — Tom DeMarco — [[content/book/slack-getting-past-burnout-busywork-and-the-myth|pagina]] — tech-lead, agile-software-development, lean
- **Talking with Tech Leads** — Patrick Kua — [[content/book/talking-with-tech-leads-from-novices-to|pagina]] — tech-lead, leadership
- **Test-Driven Development: By Example** — Kent Beck — [[content/book/test-driven-development-by-example|pagina]] — agile-software-development
- **The DevOps Handbook** — Gene Kim, Jez Humble, Patrick Debois, John Willis — [[content/book/the-devops-handbook|pagina]] — agile-software-development, devops
- **The Phoenix Project** — Gene Kim, Kevin Behr, George Spafford — [[content/book/the-phoenix-project-a-novel-about-it-devops-and|pagina]] — agile-software-development, devops
- **When Will It Be Done?** — Daniel S. Vacanti — [[content/book/when-will-it-be-done|pagina]] — tech-lead, product-development
- **Working Backwards** — Colin Bryar, Bill Carr — [[content/book/working-backwards|pagina]] — product-development

### Article (33)

- **10 Things You Would Expect From a Technical Lead** — [[content/article/10-things-you-would-expect-from-a-technical-lead|pagina]] — communication, tech-lead
- **20 Principles To Learn and Lead** — [[content/article/20-principles-to-learn-and-lead|pagina]] — tech-lead, leadership, learning
- **#36 — Le 36 Skill di Claude che cambieranno il tuo modo di lavorare** — [[content/article/36-le-36-skill-di-claude-che-cambieranno-il-tuo|pagina]] — ai, technology
- **A Random Walk** — [[content/article/a-random-walk|pagina]] — personal-growth
- **AI Systems Engineering Patterns** — [[content/article/ai-systems-engineering-patterns|pagina]] — ai, technology, ai-development
- **Book Summary for Lean Software Development: An Agile Toolkit** — [[content/article/book-summary-for-lean-software-development-an|pagina]] — agile-software-development, lean
- **Breaking Through That First TDD Block** — [[content/article/breaking-through-that-first-tdd-block|pagina]] — agile-software-development
- **Che cos'è una feature factory?** — [[content/article/che-cos-e-una-feature-factory|pagina]] — product-development
- **Claude Cowork: The Complete Guide for PMs** — [[content/article/claude-cowork-the-complete-guide-for-pms|pagina]] — technology, ai-development
- **Code Is Cheap Now. Software Isn't.** — [[content/article/code-is-cheap-now-software-isnt|pagina]] — ai, technology, software-development
- **Cos'è il debito tecnico e come affrontarlo in modo Agile** — [[content/article/cose-il-debito-tecnico-e-come-affrontarlo-in-modo|pagina]] — tech-lead, agile-software-development, tech-debt, technical-excellence
- **Doerr's Law on Product Teams** — [[content/article/doerr-s-law-on-product-teams|pagina]] — agile-software-development, product-development, product-teams
- **DORA — Balancing AI Tensions** — [[content/article/dora-balancing-ai-tensions-moving-from-ai|pagina]] — ai, agile-software-development, dora
- **How LennyRPG Was Built in 8 Hours** — [[content/article/how-lennyrpg-was-built-in-8-hours-by-ben-shih-in|pagina]] — technology, ai-development
- **Humans and Agents in Software Engineering Loops** — [[content/article/humans-and-agents-in-software-engineering-loops|pagina]] — ai, agile-software-development
- **Il peggior modo di fare una riunione** — [[content/article/il-peggior-modo-di-fare-una-riunione|pagina]] — communication, meetings
- **Is High Quality Software Worth the Cost?** — [[content/article/is-high-quality-software-worth-the-cost|pagina]] — agile-software-development, product-development, technical-excellence
- **Learned (And Actual) Helplessness** — [[content/article/learned-and-actual-helplessness|pagina]] — agile-software-development
- **"okay" vs excellent engineering teams** — [[content/article/okay-vs-excellent-engineering-teams|pagina]] — tech-lead, leadership
- **Productivity** — [[content/article/productivity|pagina]] — agile-software-development
- **Psychological Safety - PlasticBlog** — [[content/article/psychological-safety-plasticblog|pagina]] — leadership, psychological-safety
- **Six Rules for Designing Company Goals** — [[content/article/six-rules-for-designing-company-goals|pagina]] — leadership
- **Tech Debt Explained to Everyone** — [[content/article/tech-debt-explained-to-everyone|pagina]] — tech-lead, agile-software-development, tech-debt, technical-excellence
- **The Developer Productivity Trap** — [[content/article/the-developer-productivity-trap|pagina]] — software-development
- **The Elephant Carpaccio** — [[content/article/the-elephant-carpaccio-intre|pagina]] — agile-software-development
- **The Engineer → Executive Translation Layer** — [[content/article/the-engineer-executive-translation-layer|pagina]] — leadership
- **The Golden Rules of Agent-First Product Engineering** — [[content/article/the-golden-rules-of-agent-first-product|pagina]] — ai, agile-software-development
- **The Product Engineer Company: How Portkey Works** — [[content/article/the-product-engineer-company-how-portkey-works|pagina]] — product-development
- **The Scientifically Proven, Step-by-Step Guide to Having a Breakthrough Conversation Across Party Lines** — [[content/article/the-scientifically-proven-step-by-step-guide-to|pagina]] — communication, conflict-resolution
- **The Three Ways: The Principles Underpinning DevOps** — [[content/article/the-three-ways-the-principles-underpinning-devops|pagina]] — agile-software-development, devops
- **The Three-Year Bill: Why Fast Teams Slow Down** — [[content/article/the-three-year-bill-why-fast-teams-slow-down|pagina]] — agile-software-development
- **Trunk-Based Development And Branch By Abstraction** — [[content/article/trunk-based-development-and-branch-by-abstraction|pagina]] — agile-software-development, trunk-based-development
- **Tutto quello che ho imparato su Team Topologies** — [[content/article/tutto-quello-che-ho-imparato-su-team-topologies|pagina]] — agile-software-development

### Video (9)

- **11 principi fondamentali che dovresti tenere a mente ogni giorno** — [[content/video/11-principi-fondamentali-che-dovresti-tenere-a|pagina]] — leadership, personal-growth
- **Async Code Reviews Are Choking Your Company's Throughput** — [[content/video/async-code-reviews-are-choking-your-companys|pagina]] — agile-software-development, extreme-programming
- **Claude Cowork: la guida completa per creare i tuoi agenti AI personali da zero** — [[content/video/claude-cowork-la-guida-completa-per-creare-i-tuoi|pagina]] — ai, technology
- **Come (non) usare l'AI per programmare? — Reaction a @antirez** — [[content/video/come-non-usare-l-ai-per-programmare-reaction-a|pagina]] — ai, technology, ai-development
- **Everything We Learned at Stanford Business School in 19 Minutes** — [[content/video/everything-we-learned-at-stanford-business-school|pagina]] — personal-growth, business-development
- **Greg Young — CQRS and Event Sourcing — Code on the Beach 2014** — [[content/video/greg-young-cqrs-and-event-sourcing-code-on-the|pagina]] — agile-software-development, technical-excellence, ddd
- **L'effetto sistemico di eXtreme Programming con Nicola Moretto** — [[content/video/l-effetto-sistemico-di-extreme-programming-con|pagina]] — agile-software-development, extreme-programming
- **Who Should Enable TDD? — Scrum Master | Line Manager | Tech Lead** — [[content/video/who-should-enable-tdd-scrum-master-line-manager|pagina]] — agile-software-development
- **XPUG Bg — Luca Giovenzana, Bounded Context** — [[content/video/xpug-bg-27-01-2026-luca-giovenzana-bounded|pagina]] — agile-software-development, ddd

### Conference (3)

- **Italian Agile Days 2021** — [[content/conference/italian-agile-days-2021|pagina]] — agile-software-development
- **PHP Day 2024** — [[content/conference/php-day-2024|pagina]] — software-development, php
- **TDD Conference 2021** — [[content/conference/tdd-conference-2021|pagina]] — agile-software-development, technical-excellence

### Podcast (3)

- **01.05 L'Essenziale 05 — Il Progetto Aristotele** — [[content/podcast/01-05-l-essenziale-05-il-progetto-aristotele-le|pagina]] — leadership, agile-software-development, psychological-safety
- **Ep. 78 — Hexagonal Architecture con Alessandro Minoccheri** — [[content/podcast/ep-78-hexagonal-architecture-con-alessandro|pagina]] — agile-software-development, technical-excellence, architecture
- **Gitbar** — [[content/podcast/gitbar|pagina]] — _non taggato_

### Course (2)

- **AI for TypeScript Developers** — [[content/course/ai-for-typescript-developers|pagina]] — technology, ai-development, software-development
- **Eventstorming Facilitation** — [[content/course/eventstorming-facilitation|pagina]] — agile-software-development, product-development, eventstorming

## Vedi anche

- [[knowledge-archive|Archivio Recall importato]]
- [[completed|Completati]]
